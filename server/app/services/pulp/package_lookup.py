...
async def package_lookup(
    ...
    async with RepositoryApi() as api:
        params["repository_version"] = await api.latest_version_href(repo)
    if release:
        ...
        if request_size < 10 and request_size > 0:
            ...
            for thing in package_queries + package_ids:
                if isinstance(thing, PackageId):
                    ...
                    results = await api.list(params=query, type=type)
            ...
            async for package in yield_all(api.list, params=params, type=type):
            ...
            for name in packages_by_name.keys():
                if name not in found_names:
                    logger.warning(
                        f"package_lookup included {name}, but was not found for {params}!"
                    )
            for id in ids:
