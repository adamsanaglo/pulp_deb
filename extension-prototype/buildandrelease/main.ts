/*
* main.ts
* 
* Will be compiled into javascript and executed when the Publishing Helper is called.
* Capable of publishing packages.
*/

/* -------------------------------------------------------------------------- */
/*                 Required Packages and Helper Functions                     */
/* -------------------------------------------------------------------------- */

import tl = require('azure-pipelines-task-lib/task');
const { execSync } = require('child_process');

interface iProfileOption {
    [key: string]: string
}

interface iProfileOptions {
    [key: string]: iProfileOption
}

interface iResponseItem {
    id: string
}

function generateConfigs(
    profile: string = "prod",
    msal_client_id: string | undefined,
    msal_SNIAuth: string | undefined) {
    
    let options: string = '';
    
    const profileOptions: iProfileOptions = {
        "prod": {
            "base-url": "https://pmc-ingest.trafficmanager.net/api/v4",
            "msal-scope": "api://d48bb382-20ec-41b9-a0ea-07758a21ccd0/.default",
            "msal-authority": "https://login.microsoftonline.com/MSAzureCloud.onmicrosoft.com"
        },
        "tuxdev": {
            "base-url": "https://tux-ingest.corp.microsoft.com/api/v4",
            "msal-scope": "api://55391a9d-3c3b-4e4a-afa6-0e49c2245175/.default",
            "msal-authority": "https://login.microsoftonline.com/Microsoft.onmicrosoft.com"
        },
        "ppe": {
            "base-url": "https://ppe-ingest.trafficmanager.net/api/v4",
            "msal-scope": "api://1ce02e3e-1bf3-4d28-8cdc-e921f824399d/.default",
            "msal-authority": "https://login.microsoftonline.com/Microsoft.onmicrosoft.com"
        }
    }

    for (const key in profileOptions[profile]) {
        options += `--${key} ${profileOptions[profile][key]} `
    }

    options += `--msal-client-id ${msal_client_id} `;
    options += `--${msal_SNIAuth}`;
    return options;
}

function getPackageID(packageIDCommand: string | undefined, msal_cert: string | undefined) {
    let response: Array<iResponseItem> | undefined;
    let parsedIDs: Array<string> = new Array();

    try {
        response = JSON.parse(execSync(packageIDCommand, {env: {'PMC_CLI_MSAL_CERT': msal_cert}}).toString());
        if (response != undefined) {
            for (const item of response) {
                parsedIDs.push(item.id);
            }
        }
    } catch (e: any) {
        tl.setResult(tl.TaskResult.Failed, `Operation failed: ${e.stdout}`);
    }

    return parsedIDs.join();
}

function runPMCCommand(runCommand: string | undefined, msal_cert: string | undefined) {
    let execSuccess: boolean = true;
    let OperationOuput: string | undefined;

    try{
        OperationOuput = execSync(runCommand, {env: {'PMC_CLI_MSAL_CERT': msal_cert}}).toString();
        console.log(`Operation succeeded: ${OperationOuput}`);
    } catch (e: any) {
        execSuccess = false;
        tl.setResult(tl.TaskResult.Failed, `Operation failed: ${e.stdout}`);
    }

    return execSuccess;
}

function repositoryCheck(repository: string | undefined) {
    if (repository == undefined) {
        return false;
    }

    const repoTypeSuffix: string | undefined = repository?.slice(repository?.lastIndexOf('-') + 1);

    return (repoTypeSuffix == 'yum' || repoTypeSuffix == 'apt');
}

function inputCheck(
    sourceDir: string | undefined,
    msal_cert: string | undefined,
    msal_client_id: string | undefined,
    msal_SNIAuth: string | undefined,
    repository: string | undefined) {

    if (sourceDir == undefined) {
        tl.setResult(tl.TaskResult.Failed, 'Need package directory path');
        return;
    } else if (msal_cert == undefined) {
        tl.setResult(tl.TaskResult.Failed, 'Missing cert');
        return;
    } else if (msal_client_id == undefined) {
        tl.setResult(tl.TaskResult.Failed, 'Need client ID');
        return;
    } else if (msal_SNIAuth == undefined) {
        tl.setResult(tl.TaskResult.Failed, 'Must indicate whether Subject Name Issuer Auth should be used');
        return;
    } else if (!repositoryCheck(repository)) {
        tl.setResult(tl.TaskResult.Failed, 'Repository name must be entered with the appropriate type suffix -yum or -apt');
    }
}

/* -------------------------------------------------------------------------- */
/*                          Main Executing Function                           */
/* -------------------------------------------------------------------------- */

function main() {
    try {
        // Retrieve user inputs
        const sourceDir: string | undefined = tl.getPathInput('package_path', false, false);
        const profile: string | undefined = tl.getInput('profile', true);
        const msal_cert: string | undefined = tl.getInput('msal_cert', true);
        const msal_client_id: string | undefined = tl.getInput('msal_client_id', true);
        const msal_SNIAuth: string | undefined = tl.getInput('msal_SNIAuth', true);
        const repository: string | undefined = tl.getInput('repository', true);

        // Checks that all inputs are present
        inputCheck(sourceDir, msal_cert, msal_client_id, msal_SNIAuth, repository);

        const dockerImage = 'mcr.microsoft.com/pmc/pmc-cli';

        const dockerCommand: string = `docker run -e PMC_CLI_MSAL_CERT -v ${sourceDir}:${sourceDir} ${dockerImage}`;
        const cliOptions: string = generateConfigs(profile, msal_client_id, msal_SNIAuth);
        
        // Build command to retrieve a package ID for the package to upload
        const packageIDCommand: string = `${dockerCommand} ${cliOptions} package upload ${sourceDir}`;
        const packageIDs = getPackageID(packageIDCommand, msal_cert);

        // Build command to update the repo with the package
        const updateRepoPackageCommand: string = `${dockerCommand} ${cliOptions} repo packages update ${repository} --add-packages ${packageIDs}`;
        runPMCCommand(updateRepoPackageCommand, msal_cert);

        // Build command to publish the repo
        const publishRepoCommand: string = `${dockerCommand} ${cliOptions} repo publish ${repository}`;
        runPMCCommand(publishRepoCommand, msal_cert);
    }
    catch (err: any) {
        tl.setResult(tl.TaskResult.Failed, err.message);
    }
}

main();