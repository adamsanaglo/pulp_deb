...
if [ "$2" = "--create" ]
 then
   ...
   docker exec -i db psql -U pmcserver -c "insert into account (id, oid, name, role, icm_service, icm_team, contact_email, is_enabled, created_at, last_edited) values (gen_random_uuid(), '$account_id', 'dev', '$1', 'dev', 'dev', 'dev@user.com', 't', now(), now())"
 else
   docker exec -i db psql -U pmcserver -c "update account set role = '$1' where oid = '$account_id'"
fi
